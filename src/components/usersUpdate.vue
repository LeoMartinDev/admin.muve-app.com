<template>
  <b-modal id="modal1" title="Create an user" @hidden="onHidden" @cancel="onCancel" @ok="onOk" :visible="true">
    ...
    <div v-show="isLoading || error || success" class="pt-4">
      ...
      <b-alert :show="!isLoading && success !== false" variant="success">User updated!</b-alert>
    </div>
    <template slot="modal-footer">
      <b-btn @click.prevent="onCancel">Cancel</b-btn>
      <b-btn @click.prevent="onOk" variant="primary" :disabled="$v.form.$invalid">Update</b-btn>
    </template>
...
<script>
  ...
  export default {
    ...
    store: UsersStore,
    created() {
      //this.form = this.currentUserUpdatable;
    },
    data() {
      return {
        error: false,
    ...
    methods: {
      ...
      onOk() {
        this.success = false;
        this.isLoading = true;
        console.log('onOk : form ::: ', this.form)
        this.updateUser({id: this.$route.params.id, user: this.form })
          .then(() => this.success = true)
          .catch(error => {console.error(error);this.error = error})
